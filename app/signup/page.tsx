...
import { AnalyticsIcon, ArchiveIcon, BrandMarkIcon, CheckIcon, LinkedInIcon, VoiceIcon } from "@/components/ui/qalam-icons"
      ...
      <div className="relative order-2 hidden w-1/2 flex-col justify-between overflow-hidden bg-teal-800 p-12 lg:flex">
        ...
        <div className="relative z-10 space-y-8">
          ...
          <div className="grid grid-cols-1 gap-4">
            {[
              { icon: VoiceIcon, label: "Voice Profile", value: "Train from your own posts" },
              { icon: ArchiveIcon, label: "Post archive", value: "Keep drafts, versions, and wins" },
              { icon: AnalyticsIcon, label: "Performance loop", value: "Track what earns attention" },
            ].map((stat) => {
      ...
      <div className="order-1 flex flex-1 items-center justify-center bg-white px-6 py-12">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }} className="w-full max-w-[420px]">
          ...
          <div className="mb-7 flex flex-col gap-3">
            <button className="flex w-full items-center justify-center gap-3 rounded-xl border border-zinc-200 py-3 text-sm font-semibold text-zinc-700 transition-all hover:border-zinc-300 hover:bg-zinc-50">
            ...
            <button className="flex w-full items-center justify-center gap-3 rounded-xl bg-[#0A66C2] py-3 text-sm font-semibold text-white transition-colors hover:bg-[#085fa8]">
          ...
          <div className="mb-7 flex items-center gap-3">
            ...
            <span className="text-xs font-medium text-zinc-400">or sign up with email</span>
            <div className="h-px flex-1 bg-zinc-100" />
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div>
              ...
              <input type="password" required minLength={8} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min. 8 characters" className="w-full rounded-xl border border-zinc-200 px-4 py-3 text-sm text-zinc-900 placeholder-zinc-400 transition-all focus:border-teal/50 focus:outline-none focus:ring-2 focus:ring-teal/30" />
            </div>
